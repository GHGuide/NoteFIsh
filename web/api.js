...
export const api = {
  bootstrap: async () => { const [setup, voices, settings, calls, session] = await Promise.all([request('/status'), request('/voices?archived=true'), request('/settings'), request('/calls'), request('/session')]); return { setup, ...voices, ...settings, ...calls, session }; },
