...
// different moments. Behaviour is the old Desk's (recorder, PTT keys, seats,
// invitations, ticket saving); the clothes are design/build.py desk_screen()
// and design/moments.py desk_ringing / desk_ticket / desk_ended / seat_screen.
...
import { Toolbar, Title, Tabs, Body, Col, Label, Dot, Chip, Row, Btn, TextBtn, Pill, SelectPill, Field, Switch, Wave, Typing, Empty, Spinner, Ask } from '../shell.jsx';
...
export default function DeskPage({ data, navigate, route, setError, setNotice, saveOwned, owned, updateCall, connection, invitation, setInvitation, focus, setFocus, enableAudio, clearAudio, setAudioMuted, hasFloor, seat, partialCaption }) {
  ...
  const title = !call ? 'Desk' : ended ? 'Call ended' : ringing ? 'Incoming call' : call.from || 'Caller';
  const sub = !call ? (needsSeat ? 'Nobody answering yet' : voice ? 'Ready for a call' : 'Choose a voice first')
  ...
  const emptyNote = needsSeat ? 'Choose your name in the sidebar to start receiving calls.' : active ? `What they say appears here in ${languageName(call.agentLanguage || agentLanguage)}, as they speak.` : ended ? 'Nothing was said on this call.' : voice ? 'Create a call link below, or wait for the line to ring.' : 'Choose a voice, then create a call link.';
  ...
  return <>
    ...
    <Tabs items={ended ? [{ key: 'notes', label: 'Notes' }, { key: 'transcript', label: 'Transcript' }] : [{ key: 'captions', label: 'Captions' }, { key: 'notes', label: 'Notes' }]} value={tab} onChange={setTab} />
    ...
    {!call && (tab === 'notes' ? notes : captions)}
    {!call && !needsSeat && tab !== 'notes' && <Col className="desk-invite">
      {!voice ? <div className="ds-info">Choose a voice first<TextBtn onClick={() => navigate('/voice')}>Open Voice<ChevronRight size={12} /></TextBtn></div>
        : invitation && !inviteExpired ? <>
          ...
          <span className="ds-note">{inviteExpired ? 'Your previous link expired. Create a fresh one.' : !browserReady ? (connection !== 'connected' ? 'Reconnecting to the desk…' : data.setup?.blockers?.[0] || 'Finishing call setup…') : 'The caller opens it on their phone and taps Call.'}</span>
        </div>}
      {voice && <p className="ds-note">Try a short conversation: they ask in {customerLanguage === 'auto' ? 'their language' : languageName(customerLanguage)} when the delivery arrives; you answer in {languageName(agentLanguage)}. Headphones on both devices.</p>}
    </Col>}
