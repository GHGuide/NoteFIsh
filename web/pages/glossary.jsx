...
export default function GlossaryPage({ data, saveSettings }) {
  ...
  return <>
    <Toolbar />
    <Title title="Glossary" sub="Words that stay exactly as written, in every language" />
    <Tabs items={TABS} value={tab} onChange={setTab} />
    <ReadBar><Plus size={14} /><input value={draft} maxLength={120} placeholder="Add a word or phrase" aria-label="Add a word or phrase" disabled={list.length >= 200} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') add(); }} /></ReadBar>
    <Body>
      {shown.length ? shown.map(item => <Row key={item.id} main={item.term} sub={sub(item)} right={<>
