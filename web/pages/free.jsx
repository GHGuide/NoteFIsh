...
export default function FreeMonth({ onClose, name, setNotice }) {
  ...
  return <Sheet title="Get a free month." intro="Share NoteFish with a friend. They get a month free, and so do you." onClose={onClose}>
    ...
    {tab === 0 ? <div className="box">
      <div className="ds-ticket"><span className="fish"><Puff variant="fish" color="#F3F1EC" size={128} /></span><span className="perf" /><span className="hole top" /><span className="hole bottom" /><div className="text"><small>NOTEFISH</small><strong>One month<br />free.</strong><span><i />A gift from {name?.trim().split(/\s+/)[0] || 'your desk'}</span></div></div>
