...
export default function AuthPage({ users = 0, open = true, local = false, invite = null, onSignedIn, onSkip }) {
  ...
  const submit = async event => {
    ...
    try {
      const result = mode === 'signup' ? await api.signUp({ name: form.name, email: form.email, password: form.password, ...(invite ? { invite: invite.token } : {}) }) : await api.signIn({ email: form.email, password: form.password });
  ...
  return <div className="au">
    ...
    <section className="au-right">
      <form className="au-form" onSubmit={submit}>
        <div className="au-head"><h2>{invite ? `Join as ${invite.name || 'yourself'}` : signup ? 'Create your desk' : 'Sign in'}</h2><p>{invite ? (invite.error ? invite.error : 'You have a seat on this desk. Set a password and you are in.') : signup ? 'Free to try on one desk. Add a floor when the team grows.' : 'Your desk, your voice, your calls.'}</p></div>
        {invite?.error && <p className="au-error" role="alert">Ask whoever runs the desk for a new invitation.</p>}
        ...
        <label className="au-field"><span>Work email</span><input type="email" autoComplete="email" placeholder="nina@acme.example" value={form.email} onChange={set('email')} required readOnly={!!invite} /></label>
