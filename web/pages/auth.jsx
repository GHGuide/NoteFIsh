...
export default function AuthPage({ users = 0, local = false, onSignedIn, onSkip }) {
  const [mode, setMode] = useState(users ? 'signin' : 'signup');
  ...
  return <div className="au">
    ...
    <section className="au-right">
      <form className="au-form" onSubmit={submit}>
        <div className="au-head"><h2>{signup ? 'Create your desk' : 'Sign in'}</h2><p>{signup ? 'Free to try on one desk. Add a floor when the team grows.' : 'Your desk, your voice, your calls.'}</p></div>
        ...
        <p className="au-switch">{signup ? <>Already have a desk? <a href="#signin" onClick={event => { event.preventDefault(); setMode('signin'); setError(''); }}>Sign in</a></> : <>New here? <a href="#signup" onClick={event => { event.preventDefault(); setMode('signup'); setError(''); }}>Create your desk</a></>}</p>
      </form>
