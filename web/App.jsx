...
function Enroll({ data, navigate, setError, setNotice, refreshVoices, saveSettings, sharedDemo, trainingNotes }) {
  ...
  const [savedVoice, setCreatedVoice] = useState(null);
  ...
  const submit = async event => {
    ...
    try { const payload = new FormData(); for (const [key, value] of Object.entries(form)) payload.append(key, String(value)); payload.append('audio', sample.blob, sample.name); const result = await api.createVoice(payload); setCreatedVoice(result.voice); await refreshVoices(result.voice); setNotice(`${result.voice.name} is saved in your voice library.`); }
    catch (failure) { setError(failure.message); }
    finally { setBusy(false); }
  };
  const useCreatedVoice = async () => {
    if (await saveSettings({ voiceId: createdVoice.id })) { setNotice(`${createdVoice.name} is your call desk voice.`); navigate('/desk'); }
  };
  if (createdVoice) return <div className="page create-page"><div className="creation-success">
    <VoiceAvatar name={createdVoice.name} large /><span className="eyebrow">SAVED TO YOUR VOICE LIBRARY</span>
    <h1>{createdVoice.name}</h1>
    <p>{createdVoice.status === 'ready' ? 'Your voice is ready. Listen to a preview, then use it for translated replies at the call desk.' : createdVoice.status === 'failed' ? createdVoice.error || 'Fish could not complete this voice. Its name and details are saved in your library.' : 'Your named voice is saved. Fish Audio is creating it now; you can leave this page and return to your library.'}</p>
    <StatusPill status={isArchived(createdVoice) ? 'archived' : createdVoice.status} />
    {createdVoice.status === 'training' && trainingNotes[createdVoice.id] && <p className="voice-training-note" role="status">{trainingNotes[createdVoice.id]}</p>}
    <div className="success-actions">{createdVoice.status === 'ready' && !isArchived(createdVoice) ? <><button className="button secondary" onClick={() => setPreviewCreated(true)}><Play size={15} />Preview voice</button><button className="button primary" onClick={useCreatedVoice}>Use at call desk <ArrowRight size={16} /></button></> : <button className="button primary" onClick={() => navigate('/voices')}>Open voice library <ArrowRight size={16} /></button>}</div>
    <div className="success-links">{createdVoice.status === 'ready' && <button className="text-button" onClick={() => navigate('/voices')}><Library size={14} />Voice library</button>}<button className="text-button" onClick={() => { setCreatedVoice(null); setPreviewCreated(false); setSample(null); setAudioUrl(''); setForm({ name: '', description: '', transcript: '', language: 'en', consent: false }); }}><Plus size={14} />Create another voice</button></div>
    {sharedDemo && <p className="shared-demo-note">This demo shares one library. Anyone with the site address can use and manage voices saved here.</p>}
...
function Desk({ data, navigate, setError, setNotice, saveSettings, updateCall, connection, audioReady, enableAudio, clearAudio, setAudioMuted }) {
  ...
  return <div className={`page desk-page ${active || ringing ? 'has-call' : ''}`}><div className="page-heading horizontal"><div><span className="eyebrow">REAL PEOPLE. REAL CONVERSATIONS.</span><h1>Your call desk.</h1><p>Listen in your language. Reply in theirs.</p></div><div className={`desk-connection ${connection === 'connected' ? 'connected' : ''}`}><span />{connection === 'connected' ? 'Desk connected' : 'Reconnecting…'}</div></div>
    <div className="desk-settings"><div className="voice-select desk-voice-field"><label className="field"><span>Your voice</span><div className="select-wrap"><AudioLines size={16} /><select value={data.settings.voiceId || ''} onChange={event => saveSettings({ voiceId: event.target.value || null })} disabled={processing || !!busy}><option value="">Choose a voice</option>{readyVoices.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><ChevronDown size={14} /></div></label><div className="desk-voice-actions"><button type="button" className="text-button" disabled={active || ringing} onClick={() => navigate('/enroll')}><Plus size={13} />Create voice</button><button type="button" className="text-button" onClick={() => navigate('/voices')}><Library size={13} />Voice library</button></div></div><LanguageSelect label="Your language" value={data.settings.agentLanguage} onChange={agentLanguage => saveSettings({ agentLanguage })} disabled={processing || !!busy} /><div className="language-direction"><ArrowRight size={18} /></div><LanguageSelect label="Caller’s language" value={data.settings.customerLanguage} onChange={customerLanguage => saveSettings({ customerLanguage })} disabled={processing || !!busy} /></div>
